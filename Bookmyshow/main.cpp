...
// Cancels a block of seats if they are all booked
// Cancels a block of seats and shows trace table before and after
void cancelBlockSeatsWithTrace() {
    int row, startCol, count;
    cout << "Enter row, starting column, and number of seats to cancel (0-based): ";
    cin >> row >> startCol >> count;
    ...
    if (row >= 0 && row < ROWS && startCol >= 0 && startCol + count <= COLS) {
        ...
        if (canCancel) {
            ...
            cout << "Block cancelled successfully.\n";
        } else {
            cout << "Cancellation failed: one or more seats are not booked.\n";
        }

        cout << "\n--- Trace: After Cancellation ---\n";
        for (int j = 0; j < COLS; j++) {
            cout << seats[row][j] << " ";
        }
        cout << "\n";
...
int main() {
    ...
    while (true) {
        ...
        cout << "5. Cancel a Block Seat Booking\n";
        cout << "6. Display Available Seats Only\n";
        cout << "7. Exit\n";
        cout << "Enter your choice (1 to 7) to select a function: ";
        cin >> choice;
        ...
        switch (choice) {
            ...
            case 5: cancelBlockSeatsWithTrace(); break;
            case 6: displayAvailableSeats(); break;
            case 7:
